  ...
  Il lui est également donné la possibilité d'assigner un nouveau balai à tel ou tel joueur.
    ...
    \subsubsection{Assigner un balai à un joueur}
      \begin{itemize}
        ...
        \item \textbf{Cas général} : Le client souhaite changer le balai d’un joueur. Les balais ont des bonus particuliers qui permettent d’augmenter certains attributs des joueurs. Le joueur possèdera un nouveau balai\index{balai}. L’ancien balai ne sera plus assigné à un joueur mais reste disponible.
        \item \textbf{Cas exceptionnels} :
          \begin{itemize}
    ...
    \subsubsection{Sélectionner balai}
      \begin{itemize}
        ...
        \item \textbf{Pré-conditions} : \index{balai}Le balai doit être disponible, c’est-à-dire qu’il ne doit pas déjà être assigné à un joueur.
        \item \textbf{Post-conditions} : Néant.
  ...
  il y a aussi des balais qui octroient des bonus à certaines capacités des joueurs.
    ...
    \subsubsection{Faire des ventes}
      \begin{itemize}
        ...
        \item \textbf{Cas général} : Le client souhaite vendre un joueur ou un balai s’il n’en a plus besoin ou si ses finances vont mal. \index{balai}\index{joueur}
        \item \textbf{Cas exceptionnels} : Néant.
      \end{itemize}
  ...
  C'est via le \gls{stade} \index{bâtiment!Stade} que le Manager peut lancer un match lorsque c'est un jour de match. Il devra alors choisir les joueurs qui participeront à la rencontre. Une fois le match terminé, il pourra voir les résultats de ce dernier, ainsi que ce qu'il a gagné, ou perdu (argent et popularité).
    \subsubsection{Jouer match}
      \begin{itemize}
        ...
        \item \textbf{Post-conditions} : Néant.
        \item \textbf{Cas général} : Pour savoir si c'est un jour de match, on vérifie en checkant le \gls{calendrier} \index{calendrier} des matches. S'il y a bien un match de prévu, le client est connecté en même temps que son adversaire du jour. Ils doivent donc jouer le match dont la date a été prédéterminée.
        \item \textbf{Cas exceptionnels} : 
          \begin{itemize}
